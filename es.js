/* Glow Lab Protocols - Spanish translation pack */
window.GLP_ES = {
 "ui": {
  "Free Priority Shipping on All Orders Over $250": "Envío prioritario gratis en pedidos superiores a $250",
  "Home": "Inicio",
  "Peptides": "Péptidos",
  "COAs": "Certificados",
  "Affiliates": "Afiliados",
  "Account": "Mi cuenta",
  "Order": "Pedido",
  "Advanced Peptide Science": "Ciencia avanzada de péptidos",
  "Research.": "Investigación.",
  "Quality.": "Calidad.",
  "Results.": "Resultados.",
  "High-quality research peptides for advanced studies. All products are for research use only.": "Péptidos de investigación de alta calidad para estudios avanzados. Todos los productos son exclusivamente para uso en investigación.",
  "Shop the Catalog": "Ver el catálogo",
  "99%+ Purity": "Pureza 99 %+",
  "Guaranteed Quality": "Calidad garantizada",
  "Third-Party Tested": "Analizado por terceros",
  "Every Batch": "Cada lote",
  "Priority Shipping": "Envío prioritario",
  "Fast & Secure": "Rápido y seguro",
  "Discreet Packaging": "Empaque discreto",
  "Secure & Private": "Seguro y privado",
  "Complete Catalog": "Catálogo completo",
  "Research Peptides": "Péptidos de investigación",
  "View & Order": "Ver y pedir",
  "Pre-Order": "Preventa",
  "PRE-ORDER": "PREVENTA",
  "Priority Shipping: $15.00": "Envío prioritario: $15.00",
  "Fast · Secure · Discreet · Free over $250": "Rápido · Seguro · Discreto · Gratis en pedidos +$250",
  "Skincare & Topicals": "Cuidado de la piel y tópicos",
  "Glow Topicals": "Tópicos Glow",
  "Science Driven. Quality Focused. Research Powered.": "Impulsados por la ciencia. Enfocados en la calidad. Respaldados por la investigación.",
  "Glow Lab Protocols is dedicated to advancing scientific research by delivering premium-grade peptides with unmatched quality and transparency.": "Glow Lab Protocols se dedica a impulsar la investigación científica ofreciendo péptidos de grado premium con una calidad y transparencia inigualables.",
  "Premium Quality": "Calidad premium",
  "Only the highest purity": "Solo la más alta pureza",
  "Transparent Testing": "Análisis transparentes",
  "Full transparency, every batch": "Transparencia total, en cada lote",
  "Research Grade": "Grado investigación",
  "For advanced research": "Para investigación avanzada",
  "Secure & Discreet": "Seguro y discreto",
  "Discreet packaging & fast shipping": "Empaque discreto y envío rápido",
  "Research Use Only. Not for Human Consumption.": "Solo para uso en investigación. No apto para consumo humano.",
  "Research Guides:": "Guías de investigación:",
  "BPC-157 Guide": "Guía de BPC-157",
  "Reconstituting Peptides": "Reconstitución de péptidos",
  "Storage & Handling": "Almacenamiento y manejo",
  "Terms & Conditions": "Términos y condiciones",
  "Certificates of Analysis": "Certificados de análisis",
  "Affiliate Program": "Programa de afiliados",
  "Your Account:": "Tu cuenta:",
  "Sign In / Create Account": "Iniciar sesión / Crear cuenta",
  "Order History": "Historial de pedidos",
  "All products are sold strictly for laboratory and research use only — not for human consumption. Please confirm the following to enter.": "Todos los productos se venden estrictamente para uso de laboratorio e investigación — no para consumo humano. Confirma lo siguiente para entrar.",
  "I am 18 years of age or older.": "Tengo 18 años de edad o más.",
  "I have read and agree to the": "He leído y acepto los",
  "Enter Site": "Entrar al sitio",
  "Research use only. Not for human consumption.": "Solo para uso en investigación. No apto para consumo humano.",
  "External research / cosmetic use only. Not for injection or ingestion.": "Solo para uso externo en investigación o cosmético. No inyectar ni ingerir.",
  "← Back to catalog": "← Volver al catálogo",
  "Choose Size": "Elige el tamaño",
  "Add to Order": "Agregar al pedido",
  "Description": "Descripción",
  "Key Research Notes": "Notas clave de investigación",
  "Certificate of Analysis": "Certificado de análisis",
  "Independently third-party tested by Horizon Analytical.": "Analizado de forma independiente por Horizon Analytical.",
  "Purity": "Pureza",
  "Endotoxin": "Endotoxinas",
  "COAs are supplied by our vendor for the corresponding compound/batch. See": "Los certificados los suministra nuestro proveedor para el compuesto o lote correspondiente. Consulta",
  "Specifications": "Especificaciones",
  "Research Peptide": "Péptido de investigación",
  "Topical Serum": "Sérum tópico",
  "Under-Eye Serum": "Sérum para contorno de ojos",
  "Topical Mist": "Bruma tópica",
  "How to use": "Modo de uso",
  "Composition": "Composición",
  "Formula": "Fórmula",
  "Form": "Presentación",
  "Storage": "Almacenamiento",
  "Molar mass": "Masa molar",
  "Sequence": "Secuencia",
  "Key actives": "Activos principales",
  "Key active": "Activo principal",
  "Size": "Tamaño",
  "Type": "Tipo",
  "Base": "Base",
  "Methionine, Inositol, Choline + Cyanocobalamin (B-12) in bacteriostatic water": "Metionina, inositol, colina + cianocobalamina (B-12) en agua bacteriostática",
  "Blend — see COA": "Mezcla — ver certificado",
  "Blend": "Mezcla",
  "See COA": "Ver certificado",
  "Clear sterile liquid, 10 mL vial": "Líquido estéril transparente, vial de 10 mL",
  "Clear sterile liquid, 30 mL vial": "Líquido estéril transparente, vial de 30 mL",
  "Refrigerate 2–8 °C": "Refrigerar 2–8 °C",
  "Water for injection + 0.9% benzyl alcohol": "Agua para inyección + alcohol bencílico al 0,9 %",
  "Room temperature; keep sealed & sterile": "Temperatura ambiente; mantener sellado y estéril",
  "Lyophilized powder": "Polvo liofilizado",
  "Blue lyophilized powder": "Polvo liofilizado azul",
  "−20 °C (2–8 °C reconstituted)": "−20 °C (2–8 °C una vez reconstituido)",
  "−20 °C; protect from moisture": "−20 °C; proteger de la humedad",
  "−20 °C; protect from light": "−20 °C; proteger de la luz",
  "−20 °C; protect from oxidation/light": "−20 °C; proteger de la oxidación y la luz",
  "Unverified — needs confirmation": "Sin verificar — requiere confirmación",
  "Unverified": "Sin verificar",
  "15 mL aqueous serum": "Sérum acuoso de 15 mL",
  "Cool, dark": "Lugar fresco y oscuro",
  "0.34 fl oz / 10 mL roller": "0,34 fl oz / 10 mL con aplicador roll-on",
  "Cool, dark; avoid direct sunlight": "Fresco y oscuro; evitar la luz solar directa",
  "Aqueous cosmetic mist": "Bruma cosmética acuosa",
  "Rose water / hydrosol (confirm INCI on label)": "Agua de rosas / hidrolato (confirmar INCI en la etiqueta)",
  "60 mL fine-mist spray": "Spray de bruma fina de 60 mL",
  "Cool, out of direct sun": "Fresco, fuera de la luz solar directa",
  "30 mL dropper serum": "Sérum con gotero de 30 mL",
  "0.5 fl oz / 15 mL dropper": "0,5 fl oz / 15 mL con gotero",
  "GHK-Cu, SNAP-8, caffeine, panthenol": "GHK-Cu, SNAP-8, cafeína, pantenol",
  "KPV, GHK-Cu, SNAP-8, caffeine, panthenol": "KPV, GHK-Cu, SNAP-8, cafeína, pantenol",
  "AHK-Cu 0.5%, Panthenol 2%, Niacinamide 1.5%, Caffeine 1%, Betaine 1%, Biotin 0.1%": "AHK-Cu 0,5 %, pantenol 2 %, niacinamida 1,5 %, cafeína 1 %, betaína 1 %, biotina 0,1 %",
  "Laboratory Reagent": "Reactivo de laboratorio",
  "Research References": "Referencias de investigación",
  "Peptide of the Week": "Péptido de la semana",
  "(2–8 °C in solution)": "(2–8 °C en solución)",
  "−20 °C (2–8 °C in solution)": "−20 °C (2–8 °C en solución)"
 },
 "p": {
  "lipo-c-b12": {
   "t": "Mezcla de metionina, inositol, colina y cianocobalamina",
   "d": "Una mezcla lipotrópica de investigación que combina metionina, inositol y colina (denominadas en conjunto MIC) con cianocobalamina, suministrada en agua bacteriostática. Los constituyentes individuales se han investigado en la literatura de laboratorio por su participación en el metabolismo hepático de los lípidos y en las vías de metilación de un carbono. La metionina actúa como donante de metilo en el ciclo metionina-homocisteína, la colina es precursora de la fosfatidilcolina y la betaína, y la cianocobalamina se ha caracterizado como cofactor en la actividad de la metionina sintasa.",
   "b": [
    "Constituyentes examinados en investigación preclínica sobre el metabolismo hepático de los lípidos",
    "Metionina y colina estudiadas como donantes de metilo en el metabolismo de un carbono",
    "Cianocobalamina caracterizada como cofactor en la actividad de la metionina sintasa"
   ]
  },
  "bac-water": {
   "t": "Diluyente estéril de laboratorio",
   "d": "Agua estéril que contiene alcohol bencílico al 0,9 % como conservante bacteriostático. El alcohol bencílico se ha caracterizado en la investigación antimicrobiana como un agente que inhibe la proliferación bacteriana en solución acuosa. Se suministra como reactivo estéril de laboratorio para aplicaciones de investigación.",
   "b": [
    "Alcohol bencílico caracterizado como agente bacteriostático en investigación antimicrobiana",
    "Inhibición de la proliferación microbiana en medios acuosos descrita en la literatura",
    "Suministrado como reactivo estéril de laboratorio"
   ]
  },
  "mots-c": {
   "t": "Péptido de origen mitocondrial",
   "d": "Un péptido de 16 aminoácidos codificado en el gen del ARNr 12S mitocondrial. La investigación preclínica ha caracterizado a MOTS-c como regulador de la homeostasis metabólica, y en la literatura se le describe con frecuencia como un mimético del ejercicio. Estudios in vitro y en modelos animales indican que, en condiciones de estrés metabólico, modula la señalización de AMPK y la vía antioxidante Nrf2, con efectos observados sobre el metabolismo de la glucosa y la sensibilidad a la insulina en el músculo esquelético.",
   "b": [
    "Sensibilidad a la insulina y homeostasis de la glucosa examinadas en modelos animales (Lee et al., Cell Metabolism 2015)",
    "Efectos miméticos del ejercicio reportados en ratones envejecidos (Reynolds et al., Nat Commun 2021)",
    "Examinado en investigación preclínica sobre obesidad y deterioro metabólico asociado a la edad"
   ]
  },
  "nad": {
   "t": "Coenzima redox",
   "d": "Beta-nicotinamida adenina dinucleótido, una coenzima redox central en el metabolismo energético celular y sustrato de las sirtuinas, las enzimas PARP y el CD38. Dado que las concentraciones tisulares de NAD+ disminuyen con la edad, los investigadores han examinado su disponibilidad en relación con la señalización de las sirtuinas, la actividad de las enzimas de reparación del ADN y la función mitocondrial en sistemas in vitro y en modelos animales.",
   "b": [
    "Caracterizada como coenzima en el metabolismo energético mitocondrial y celular",
    "Estudiada como sustrato en la señalización de sirtuinas y PARP y en la reparación del ADN",
    "Examinada en investigación preclínica sobre el descenso del NAD+ asociado a la edad",
    "Farmacocinética caracterizada en la literatura publicada (Grant et al., 2019)"
   ]
  },
  "ghk-cu": {
   "t": "Tripéptido de cobre",
   "d": "El complejo de cobre(II) del tripéptido glicil-L-histidil-L-lisina, un fragmento de origen plasmático cuya concentración disminuye con la edad. Su mecanismo de acción se ha investigado in vitro, donde actúa como transportador de cobre y modula vías génicas asociadas con la reparación tisular, la formación de matriz extracelular y la respuesta antioxidante. Los estudios en cultivo de fibroblastos describen un aumento en la síntesis de colágeno, elastina y factores de crecimiento asociados.",
   "b": [
    "Síntesis de colágeno y matriz extracelular examinada en cultivo de fibroblastos",
    "Contracción de heridas, angiogénesis y granulación reportadas en modelos animales",
    "Efectos antiinflamatorios y sobre la expresión de genes antioxidantes observados in vitro"
   ]
  },
  "tirz-10": {
   "t": "Agonista dual de los receptores GIP / GLP-1",
   "d": "Un péptido sintético de 39 aminoácidos caracterizado como agonista dual de los receptores GIP y GLP-1. Su modificación lipídica prolonga la vida media circulante hasta aproximadamente cinco días. Los investigadores han examinado este mecanismo incretina dual en relación con la sensibilidad a la insulina y la regulación del balance energético, tanto en modelos preclínicos como en la literatura clínica publicada.",
   "b": [
    "Criterios glucémicos examinados en la literatura de ensayos de diabetes tipo 2 (SURPASS)",
    "Criterios de peso corporal reportados en la literatura de ensayos de obesidad (SURMOUNT-1, NEJM 2022)",
    "Cambios en la sensibilidad a la insulina y la adiponectina descritos en la literatura"
   ]
  },
  "reta": {
   "t": "Agonista triple de los receptores GLP-1 / GIP / glucagón",
   "d": "Un péptido sintético lipidado caracterizado como agonista triple de los receptores GLP-1, GIP y glucagón. Los investigadores han examinado la adición del agonismo del receptor de glucagón en relación con el gasto energético y la movilización de lípidos hepáticos, junto con los mecanismos mediados por incretinas.",
   "b": [
    "Criterios de peso corporal reportados en la literatura de ensayos de obesidad de fase 2 (NEJM 2023)",
    "Criterios glucémicos examinados en investigación sobre diabetes tipo 2",
    "Efectos sobre los lípidos hepáticos y el gasto energético descritos en la literatura"
   ]
  },
  "kpv": {
   "t": "Fragmento C-terminal de alfa-MSH",
   "d": "Un tripéptido sintético (lisina-prolina-valina) correspondiente a la secuencia C-terminal de la hormona estimulante de melanocitos alfa. La investigación preclínica ha caracterizado actividad antiinflamatoria e inmunomoduladora, con trabajos publicados concentrados en modelos gastrointestinales, dermatológicos y de reparación tisular. Cada vial se fabrica con una pureza superior al 99 % y se analiza por terceros para verificar su identidad.",
   "b": [
    "Reducción de la inflamación intestinal reportada en modelos murinos de colitis (Dalmasso et al., 2008)",
    "Actividad antiinflamatoria atribuida a la modulación de NF-kB y MAPK in vitro",
    "Examinado en investigación preclínica dermatológica y de inflamación inmunomediada"
   ]
  },
  "adamax": {
   "t": "Péptido de investigación — identidad sin confirmar",
   "d": "Adamax es un péptido sintético descrito en la literatura del proveedor como perteneciente a la familia relacionada con la dihexa y examinado en contextos de investigación de laboratorio relativos a la conectividad sináptica y la señalización del BDNF. La identidad de este compuesto no ha sido confirmada de forma independiente mediante espectrometría de masas, y Glow Lab Protocols no formula ninguna afirmación sobre su estructura.",
   "b": [
    "Identidad del compuesto sin confirmar — se recomienda verificación analítica independiente",
    "Los péptidos relacionados con la dihexa se han examinado en investigación preclínica de neuroplasticidad",
    "No existe investigación publicada sobre este compuesto específico tal como se suministra"
   ]
  },
  "glutathione": {
   "t": "Tripéptido antioxidante endógeno",
   "d": "El tripéptido endógeno gamma-glutamil-cisteinil-glicina, caracterizado en la literatura como el principal antioxidante intracelular. Su grupo tiol de cisteína reduce las especies reactivas de oxígeno y participa en reacciones de conjugación de fase II, alternando entre las formas reducida (GSH) y oxidada (GSSG). La proporción GSH/GSSG se utiliza ampliamente en investigación de laboratorio como marcador del estado redox celular.",
   "b": [
    "Caracterizado como tampón redox intracelular y cofactor de conjugación de fase II",
    "Examinado en la literatura publicada sobre la enfermedad de Parkinson (Sechi 1996; Hauser 2009)",
    "Investigado en modelos de detoxificación hepática y dermatológicos"
   ]
  },
  "dsip": {
   "t": "Péptido inductor del sueño delta",
   "d": "Un nonapéptido (Trp-Ala-Gli-Gli-Asp-Ala-Ser-Gli-Glu) aislado originalmente en asociación con la actividad del sueño de ondas delta. Los investigadores han examinado su relación con la arquitectura del sueño, las respuestas del eje hipotálamo-hipófisis-suprarrenal y la señalización neuroendocrina. No se ha establecido un receptor único para este péptido en la literatura publicada.",
   "b": [
    "Arquitectura del sueño examinada en investigación preclínica y clínica publicada",
    "Efectos moduladores del estrés y neuroprotectores reportados en modelos animales",
    "Investigado en modelos neuroendocrinos, de nocicepción y de abstinencia"
   ]
  },
  "thymosin-alpha-1": {
   "t": "Péptido inmunomodulador",
   "d": "Un péptido de 28 aminoácidos caracterizado en la investigación inmunológica como modulador de la función de los linfocitos T y de la señalización inmunitaria innata. Se ha investigado en modelos de infección, inmunología y adyuvantes de vacunas, con mecanismos descritos que involucran la señalización de receptores tipo Toll y la maduración de timocitos.",
   "b": [
    "Actividad de los linfocitos T y señalización inmunitaria examinadas en investigación preclínica",
    "Investigado en modelos de inmunología y defensa del huésped"
   ]
  },
  "tesamorelin-ipamorelin": {
   "t": "Mezcla de análogo de GHRH y secretagogo de GH",
   "d": "Una mezcla de investigación que combina tesamorelina, un análogo de la hormona liberadora de hormona del crecimiento, con ipamorelina, un secretagogo selectivo de la hormona del crecimiento que actúa sobre el receptor de grelina. Ambos mecanismos se han examinado por separado en la literatura publicada en relación con el eje de la hormona del crecimiento y el IGF-1. Las concentraciones de cada componente figuran en el certificado de análisis.",
   "b": [
    "Combina un análogo de GHRH con un secretagogo selectivo de la hormona del crecimiento",
    "Señalización del eje de la hormona del crecimiento examinada en investigación preclínica"
   ]
  },
  "klow-blend": {
   "t": "Mezcla de investigación multipéptido",
   "d": "Una mezcla de investigación que contiene GHK-Cu, BPC-157, TB-500 y KPV. Cada constituyente se ha examinado de forma independiente en investigación preclínica relativa a la reparación tisular, la angiogénesis y la señalización inflamatoria. Las concentraciones de cada componente figuran en el certificado de análisis.",
   "b": [
    "Combina cuatro péptidos examinados en investigación de reparación tisular",
    "Constituyentes investigados en investigación preclínica de remodelación e inflamación"
   ]
  },
  "aicar": {
   "t": "Activador de AMPK",
   "d": "Ribonucleótido de 5-aminoimidazol-4-carboxamida, una molécula pequeña caracterizada en la literatura como activador de la proteína quinasa activada por AMP. Su mecanismo de acción se ha examinado in vitro y en modelos animales en relación con la detección de energía celular, la captación de glucosa y el metabolismo oxidativo en el músculo esquelético.",
   "b": [
    "Caracterizado como activador de AMPK en investigación metabólica",
    "Detección de energía celular y metabolismo oxidativo examinados en modelos animales"
   ]
  },
  "5-amino-1mq": {
   "t": "Inhibidor de NNMT",
   "d": "Un inhibidor de molécula pequeña de la nicotinamida N-metiltransferasa. La investigación preclínica ha examinado la inhibición de la NNMT en relación con el manejo celular de la nicotinamida y el metabolismo del NAD+, con trabajos publicados concentrados en modelos de adipocitos y músculo esquelético.",
   "b": [
    "Caracterizado como inhibidor de NNMT en investigación preclínica",
    "Efectos sobre las vías de la nicotinamida y el NAD+ examinados en modelos de adipocitos"
   ]
  },
  "ss-31": {
   "t": "Tetrapéptido dirigido a la mitocondria",
   "d": "Un tetrapéptido dirigido a la mitocondria, identificado también en la literatura como elamipretida. Su mecanismo de acción se ha caracterizado como una asociación selectiva con la cardiolipina en la membrana mitocondrial interna, y se ha investigado in vitro y en modelos animales en relación con la respiración mitocondrial y el estrés oxidativo.",
   "b": [
    "Unión a cardiolipina y asociación con la membrana mitocondrial caracterizadas in vitro",
    "Respiración mitocondrial y estrés oxidativo examinados en modelos animales"
   ]
  },
  "pt-141": {
   "t": "Agonista del receptor de melanocortina",
   "d": "Un péptido sintético agonista del receptor de melanocortina, identificado también en la literatura como bremelanotida. Los investigadores han examinado su actividad sobre los receptores MC3R y MC4R y la señalización resultante en el sistema nervioso central en modelos preclínicos.",
   "b": [
    "Caracterizado como agonista del receptor de melanocortina",
    "Señalización del sistema nervioso central examinada en modelos preclínicos"
   ]
  },
  "selank": {
   "t": "Heptapéptido análogo de la tuftsina",
   "d": "Un heptapéptido sintético análogo del inmunopéptido tuftsina. La investigación preclínica ha examinado sus efectos sobre la señalización GABAérgica y monoaminérgica, la expresión del BDNF y la conducta relacionada con la ansiedad en modelos de roedores.",
   "b": [
    "Conducta relacionada con la ansiedad examinada en modelos de roedores",
    "Expresión del BDNF y señalización de neuropéptidos investigadas en investigación preclínica"
   ]
  },
  "bpc-157": {
   "t": "Pentadecapéptido de origen gástrico",
   "d": "Un pentadecapéptido sintético derivado de una secuencia identificada en el jugo gástrico humano. La investigación preclínica ha examinado sus efectos sobre la angiogénesis, el sistema del óxido nítrico y la señalización de factores de crecimiento, con trabajos publicados concentrados en modelos gastrointestinales, tendinosos y vasculares.",
   "b": [
    "Vías de angiogénesis y reparación tisular examinadas en modelos animales",
    "Investigado en modelos de investigación gastrointestinal y tendinosa",
    "Interacción con el sistema del óxido nítrico descrita en la literatura preclínica"
   ]
  },
  "protocol-01": {
   "t": "Sérum GHK-Cu + SNAP-8 para rostro, cuello y escote",
   "d": "Un sérum sin aclarado que combina GHK-Cu (tripéptido de cobre, estudiado para el soporte del colágeno y la matriz extracelular) con SNAP-8 (acetil octapéptido-3, estudiado para suavizar el aspecto de las líneas de expresión). Formulado para rostro, cuello y escote.",
   "b": [
    "GHK-Cu: síntesis de colágeno y matriz extracelular y remodelación de la piel",
    "SNAP-8: reduce la apariencia de las líneas de expresión",
    "Formulado para la piel delicada del rostro, cuello y escote"
   ],
   "pr": "Aplica unas gotas sobre la piel limpia y seca una o dos veces al día; da toquecitos y deja absorber. Haz una prueba en una zona pequeña primero. Evita combinarlo al mismo tiempo con ácidos fuertes (vitamina C de alta concentración, AHA). Solo para uso externo."
  },
  "protocol-02": {
   "t": "Sérum GHK-Cu Copper Renewal",
   "d": "Un sérum de péptido de cobre con un solo activo, construido en torno al GHK-Cu, planteado como un tratamiento de renovación con cobre que apoya la señalización del colágeno, la elastina y el acondicionamiento antioxidante de la piel.",
   "b": [
    "Soporte de colágeno, elastina y matriz extracelular",
    "Firmeza, tono y acondicionamiento de la barrera cutánea",
    "Señalización antioxidante y de reparación de la piel"
   ],
   "pr": "Aplica unas gotas sobre el rostro y el cuello limpios una o dos veces al día; da toquecitos. Haz una prueba en una zona pequeña primero. Evita usar simultáneamente activos ácidos fuertes. Solo para uso externo."
  },
  "glow-eyes": {
   "t": "GHK-Cu + SNAP-8 + cafeína + pantenol",
   "d": "Un sérum específico para el contorno de ojos que combina GHK-Cu, SNAP-8, cafeína y pantenol en una fórmula ligera con aplicador roll-on. Diseñado para hidratar y refrescar la delicada zona de los ojos mientras favorece un aspecto más liso y descansado.",
   "b": [
    "El GHK-Cu contribuye a la investigación sobre acondicionamiento de la piel y señalización del colágeno",
    "El SNAP-8 se estudia por suavizar el aspecto de las líneas de expresión",
    "La cafeína y el pantenol ayudan a refrescar, hidratar y acondicionar el contorno de ojos"
   ],
   "pr": "Aplica una pequeña cantidad bajo los ojos con el aplicador roll-on y da toquecitos suaves. Úsalo sobre piel limpia y seca una o dos veces al día. Evita el contacto directo con los ojos. Haz una prueba en una zona pequeña primero. Solo para uso externo."
  },
  "glow-eyes-luxe-kpv": {
   "t": "Sérum avanzado para el contorno de ojos con KPV",
   "d": "La fórmula Glow Eyes elevada, enriquecida con KPV junto a GHK-Cu, SNAP-8, cafeína y pantenol. Este sérum avanzado con aplicador roll-on está diseñado para hidratar, acondicionar y refrescar la delicada zona de los ojos, favoreciendo un aspecto más liso, calmado y revitalizado.",
   "b": [
    "El KPV aporta un acondicionamiento avanzado y un efecto calmante",
    "El GHK-Cu y el SNAP-8 favorecen una piel acondicionada y de aspecto más liso",
    "La cafeína y el pantenol ayudan a refrescar e hidratar el contorno de ojos"
   ],
   "pr": "Aplica una pequeña cantidad bajo los ojos con el aplicador roll-on y da toquecitos suaves. Úsalo sobre piel limpia y seca una o dos veces al día. Evita el contacto directo con los ojos. Haz una prueba en una zona pequeña primero. Solo para uso externo."
  },
  "rose-recovery-mist": {
   "t": "Bruma facial calmante e hidratante",
   "d": "Rose Recovery Mist es una bruma facial ligera infusionada con rosa, creada para hidratar, calmar y refrescar la piel en cualquier momento del día. Rica en humectantes y botánicos calmantes, ayuda a calmar la piel irritada, enrojecida o sensibilizada, perfecta después de la limpieza, como impulso a media jornada o para recuperarse tras entrenar, dejando la piel jugosa, equilibrada y con un suave aroma a rosa.",
   "b": [
    "El agua de rosas se ha estudiado por sus propiedades calmantes y suavemente antiirritantes",
    "Un paso de tónico o bruma hidratante",
    "Capa ligera antes de los séruns; para todo tipo de piel"
   ],
   "pr": "Agita suavemente; pulveriza sobre la piel limpia con los ojos cerrados como paso hidratante, o sobre el maquillaje para refrescar. Haz una prueba si eres sensible a las fragancias. Solo para uso externo."
  },
  "cu-hair-01": {
   "t": "Sérum de GHK-Cu para cuero cabelludo y cabello",
   "d": "Cu-Hair 01 es nuestro sérum base de péptido de cobre para el cuero cabelludo. Se aplica con gotero directamente sobre el cuero cabelludo y la línea del cabello; esta fórmula de un solo activo con GHK-Cu acondiciona el cuero cabelludo y favorece un entorno folicular saludable. Es la capa base de uso diario, ideal para quien empieza una rutina con péptidos de cobre.",
   "b": [
    "Acondicionamiento perifolicular y de la matriz dérmica",
    "Señalización de factores de crecimiento para el microentorno del cuero cabelludo",
    "Sérum con gotero sin aclarado para uso localizado"
   ],
   "pr": "Separa el cabello y aplica las gotas directamente sobre el cuero cabelludo una vez al día; masajea suavemente y deja actuar. Haz una prueba en una zona pequeña primero. Solo para uso externo."
  },
  "cu-hair-02": {
   "t": "Hair Protocol 02 — sérum avanzado para el cuero cabelludo",
   "d": "Cu-Hair 02 es la versión avanzada. Combina AHK-Cu, un tripéptido de cobre de nueva generación, con una mezcla específica de pantenol, niacinamida, cafeína, betaína y biotina para hidratar, calmar y revitalizar el cuero cabelludo. Más concentrado y con más activos que Cu-Hair 01, está pensado para quienes buscan un protocolo más completo y enfocado en resultados.",
   "b": [
    "Péptido de cobre AHK-Cu más una mezcla completa de activos de apoyo",
    "Hidratación · calma · equilibrio para el cuero cabelludo",
    "Pantenol, niacinamida, cafeína, betaína y biotina"
   ],
   "pr": "Aplica las gotas directamente sobre el cuero cabelludo una vez al día; masajea suavemente y deja actuar. Se recomienda hacer una prueba en una zona pequeña. Solo para uso externo."
  }
 }
};
